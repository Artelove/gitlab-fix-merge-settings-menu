...
1. Чтобы переключиться на файл, при закрытом дереве, нужно сначала его **открыть дерево**, а для этого **подняться наверх**, **открыть дерево** открыть необходимый файл, и после просмотра нужно закрыть окно иеррархии файлов ввиду малого места на экране для комфортного просмотра мр\`а. (**долго и неудобно**)
